...
package body RFLX.RSP.Request_Message_With_Id with
  SPARK_Mode
is
   ...
   function Successor (Ctx : Context; Fld : Field) return Virtual_Field is
    ...
    with
     Pre =>
       ...
       and RFLX.RSP.Request_Message_With_Id.Well_Formed (Ctx, Fld)
       and RFLX.RSP.Request_Message_With_Id.Valid_Predecessor (Ctx, Fld);
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.RSP.Request_Message_With_Id.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.RSP.Request_Message_With_Id.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert ((if Fld = F_Kind or Fld = F_Payload then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0));
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       ...
       and then RFLX.RSP.Request_Message_With_Id.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.RSP.Request_Message_With_Id.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.RSP.Request_Message_With_Id.Composite_Field (Fld)
                 then
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.RSP.Request_Message_With_Id.Valid_Next (Ctx, Fld)
       and then Fld in F_Stack_Id | F_Kind | F_Length
       and then RFLX.RSP.Request_Message_With_Id.Valid_Value (Fld, Val)
       and then RFLX.RSP.Request_Message_With_Id.Valid_Size (Ctx, Fld, RFLX.RSP.Request_Message_With_Id.Field_Size (Ctx, Fld))
       and then RFLX.RSP.Request_Message_With_Id.Available_Space (Ctx, Fld) >= RFLX.RSP.Request_Message_With_Id.Field_Size (Ctx, Fld)
       and then RFLX.RSP.Request_Message_With_Id.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.RSP.Request_Message_With_Id.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
       and Invalid_Successor (Ctx, Fld)
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.RSP.Payload_Length) is
   begin
      Set_Scalar (Ctx, F_Length, RFLX.RSP.To_Base_Integer (Val));
   end Set_Length;
   ...
   procedure Initialize_Payload_Private (Ctx : in out Context; Length : RFLX_Types.Length) with
     Pre =>
       ...
       and then RFLX.RSP.Request_Message_With_Id.Field_First (Ctx, RFLX.RSP.Request_Message_With_Id.F_Payload) mod RFLX_Types.Byte'Size = 1,
     Post =>
       Has_Buffer (Ctx)
