...
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     ((case Fld is
          when F_Kind =>
             ...
             and Invalid (Ctx.Cursors (F_Stack_Id)),
          when F_Length =>
             Invalid (Ctx.Cursors (F_Payload)),
          when F_Payload =>
             Invalid (Ctx.Cursors (F_Stack_Id)),
          when F_Stack_Id =>
             True));
   ...
   is
      ...
      Size : constant Positive := (case Fld is
          when F_Kind | F_Length | F_Stack_Id =>
             8,
          when others =>
             Positive'Last);
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert ((if Fld = F_Stack_Id then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0));
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     ...
     Post =>
       ...
       and then (case Fld is
                    ...
                    when F_Payload =>
                       (Predecessor (Ctx, F_Stack_Id) = F_Payload
                        and Valid_Next (Ctx, F_Stack_Id)),
                    when F_Stack_Id =>
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.RSP.Request_Message.Valid_Next (Ctx, Fld)
       and then Fld in F_Kind | F_Length | F_Stack_Id
       and then RFLX.RSP.Request_Message.Valid_Value (Fld, Val)
