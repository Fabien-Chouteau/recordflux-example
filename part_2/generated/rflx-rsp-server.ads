...
is
   ...
   type State is (S_Receive_Request, S_Store, S_Answer_Success, S_Get, S_Send_Data_Answer, S_Exception_Error, S_Invalid_Packet_Error, S_Got_Answer_Packet_Error, S_Stack_Full_Error, S_Setup_Answer, S_Send_Answer, S_Final);
   ...
   procedure Get_Data (Ctx : in out Context; Stack_Id : RFLX.RSP.Stack_Identifier; RFLX_Result : out RFLX.RSP.Payload.Structure) is abstract;
...
private
   ...
   type Private_Context is
      record
         ...
         Request_Ctx : RSP.RSP_Message.Context;
   ...
   function Uninitialized (Ctx : Context'Class) return Boolean is
     (not RSP.RSP_Message.Has_Buffer (Ctx.P.Request_Ctx)
   ...
   function Global_Initialized (Ctx : Context'Class) return Boolean is
     (RSP.RSP_Message.Has_Buffer (Ctx.P.Request_Ctx)
   ...
   function Write_Buffer_Size (Ctx : Context'Class; Chan : Channel) return RFLX_Types.Length is
     ((case Chan is
          when C_Chan =>
             (case Ctx.P.Next_State is
                 when S_Receive_Request =>
                    RSP.RSP_Message.Buffer_Length (Ctx.P.Request_Ctx),
                 when others =>
                    RFLX_Types.Unreachable)));
