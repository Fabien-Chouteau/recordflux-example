...
is
   ...
   function Invalid_Successor (Ctx : Context; Fld : Field) return Boolean is
     ((case Fld is
          ...
          when F_Answer_Kind =>
             Invalid (Ctx.Cursors (F_Answer_Payload_Length))
             and Invalid (Ctx.Cursors (F_Answer_Server_Result)),
          when F_Request_Stack_Id =>
             Invalid (Ctx.Cursors (F_Request_Kind)),
          when F_Answer_Payload_Length =>
             Invalid (Ctx.Cursors (F_Answer_Payload_Data)),
          when F_Answer_Server_Result =>
             True,
          when F_Request_Kind =>
             Invalid (Ctx.Cursors (F_Request_Payload_Length)),
          when F_Answer_Payload_Data =>
             True,
          when F_Request_Payload_Length =>
             Invalid (Ctx.Cursors (F_Request_Payload_Data)),
          when F_Request_Payload_Data =>
             True));
   ...
   function Composite_Field (Fld : Field) return Boolean is
     (Fld in F_Answer_Payload_Data | F_Request_Payload_Data);
   ...
   is
      ...
      Size : constant Positive := (case Fld is
          when F_Kind | F_Answer_Kind | F_Request_Stack_Id | F_Answer_Payload_Length | F_Answer_Server_Result | F_Request_Kind | F_Request_Payload_Length =>
             8,
          when others =>
             Positive'Last);
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert ((if
                                  Fld = F_Answer_Payload_Data
                                  or Fld = F_Answer_Server_Result
                                  or Fld = F_Request_Kind
                                  or Fld = F_Request_Payload_Data
                               then
   ...
   function Get_Answer_Payload_Data (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Answer_Payload_Data).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Answer_Payload_Data).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_Answer_Payload_Data;

   function Get_Request_Payload_Data (Ctx : Context) return RFLX_Types.Bytes is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Request_Payload_Data).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Request_Payload_Data).Last);
   begin
      return Ctx.Buffer.all (First .. Last);
   end Get_Request_Payload_Data;

   procedure Get_Answer_Payload_Data (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Answer_Payload_Data).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Answer_Payload_Data).Last);
   begin
   ...
   procedure Get_Request_Payload_Data (Ctx : Context; Data : out RFLX_Types.Bytes) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Request_Payload_Data).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Request_Payload_Data).Last);
   begin
      Data := (others => RFLX_Types.Byte'First);
   ...
   procedure Generic_Get_Answer_Payload_Data (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Answer_Payload_Data).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Answer_Payload_Data).Last);
   begin
      Process_Answer_Payload_Data (Ctx.Buffer.all (First .. Last));
   end Generic_Get_Answer_Payload_Data;

   procedure Generic_Get_Request_Payload_Data (Ctx : Context) is
      First : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Request_Payload_Data).First);
      Last : constant RFLX_Types.Index := RFLX_Types.To_Index (Ctx.Cursors (F_Request_Payload_Data).Last);
   begin
      Process_Request_Payload_Data (Ctx.Buffer.all (First .. Last));
   end Generic_Get_Request_Payload_Data;
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     ...
     Post =>
       ...
       and then (case Fld is
                    ...
                    when F_Request_Payload_Length =>
                       Get_Request_Payload_Length (Ctx) = To_Actual (Val)
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.RSP.RSP_Message.Valid_Next (Ctx, Fld)
       and then Fld in F_Kind | F_Answer_Kind | F_Request_Stack_Id | F_Answer_Payload_Length | F_Answer_Server_Result | F_Request_Kind | F_Request_Payload_Length
       and then RFLX.RSP.RSP_Message.Valid_Value (Fld, Val)
   ...
   procedure Set_Answer_Payload_Length (Ctx : in out Context; Val : RFLX.RSP.Length) is
   begin
      Set_Scalar (Ctx, F_Answer_Payload_Length, RFLX.RSP.To_Base_Integer (Val));
   end Set_Answer_Payload_Length;

   procedure Set_Answer_Server_Result (Ctx : in out Context; Val : RFLX.RSP.Server_Result_Kind) is
   begin
      Set_Scalar (Ctx, F_Answer_Server_Result, RFLX.RSP.To_Base_Integer (Val));
   end Set_Answer_Server_Result;
   ...
   procedure Set_Request_Payload_Length (Ctx : in out Context; Val : RFLX.RSP.Length) is
   begin
      Set_Scalar (Ctx, F_Request_Payload_Length, RFLX.RSP.To_Base_Integer (Val));
   end Set_Request_Payload_Length;

   procedure Set_Answer_Payload_Data_Empty (Ctx : in out Context) is
      Unused_Buffer_First, Unused_Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Answer_Payload_Data, 0, 0, True, Unused_Buffer_First, Unused_Buffer_Last, Unused_Offset);
   end Set_Answer_Payload_Data_Empty;

   procedure Set_Request_Payload_Data_Empty (Ctx : in out Context) is
      Unused_Buffer_First, Unused_Buffer_Last : RFLX_Types.Index;
      Unused_Offset : RFLX_Types.Offset;
   begin
      Set (Ctx, F_Request_Payload_Data, 0, 0, True, Unused_Buffer_First, Unused_Buffer_Last, Unused_Offset);
   end Set_Request_Payload_Data_Empty;

   procedure Initialize_Answer_Payload_Data_Private (Ctx : in out Context; Length : RFLX_Types.Length) with
     Pre =>
       not Ctx'Constrained
       and then RFLX.RSP.RSP_Message.Has_Buffer (Ctx)
       ...
       and then RFLX.RSP.RSP_Message.Field_First (Ctx, RFLX.RSP.RSP_Message.F_Answer_Payload_Data) mod RFLX_Types.Byte'Size = 1,
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Answer_Payload_Data)
       and Field_Size (Ctx, F_Answer_Payload_Data) = RFLX_Types.To_Bit_Length (Length)
       and Ctx.Verified_Last = Field_Last (Ctx, F_Answer_Payload_Data)
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Answer_Payload_Data);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Answer_Payload_Data) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Answer_Payload_Data);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Answer_Payload_Data) := (State => S_Well_Formed, First => First, Last => Last, Value => 0, Predecessor => Ctx.Cursors (F_Answer_Payload_Data).Predecessor);
      Ctx.Cursors (Successor (Ctx, F_Answer_Payload_Data)) := (State => S_Invalid, Predecessor => F_Answer_Payload_Data);
   end Initialize_Answer_Payload_Data_Private;

   procedure Initialize_Answer_Payload_Data (Ctx : in out Context) is
   begin
      Initialize_Answer_Payload_Data_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Answer_Payload_Data)));
   end Initialize_Answer_Payload_Data;

   procedure Initialize_Request_Payload_Data_Private (Ctx : in out Context; Length : RFLX_Types.Length) with
     Pre =>
       not Ctx'Constrained
       and then RFLX.RSP.RSP_Message.Has_Buffer (Ctx)
       ...
       and then RFLX.RSP.RSP_Message.Field_First (Ctx, RFLX.RSP.RSP_Message.F_Request_Payload_Data) mod RFLX_Types.Byte'Size = 1,
     Post =>
       Has_Buffer (Ctx)
       and Well_Formed (Ctx, F_Request_Payload_Data)
       and Field_Size (Ctx, F_Request_Payload_Data) = RFLX_Types.To_Bit_Length (Length)
       and Ctx.Verified_Last = Field_Last (Ctx, F_Request_Payload_Data)
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
   ...
   is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Request_Payload_Data);
      Last : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Request_Payload_Data) + RFLX_Types.Bit_Length (Length) * RFLX_Types.Byte'Size - 1;
   begin
      pragma Assert (Last mod RFLX_Types.Byte'Size = 0);
      Reset_Dependent_Fields (Ctx, F_Request_Payload_Data);
      pragma Warnings (Off, "attribute Update is an obsolescent feature");
      Ctx := Ctx'Update (Verified_Last => Last, Written_Last => Last);
      pragma Warnings (On, "attribute Update is an obsolescent feature");
      Ctx.Cursors (F_Request_Payload_Data) := (State => S_Well_Formed, First => First, Last => Last, Value => 0, Predecessor => Ctx.Cursors (F_Request_Payload_Data).Predecessor);
      Ctx.Cursors (Successor (Ctx, F_Request_Payload_Data)) := (State => S_Invalid, Predecessor => F_Request_Payload_Data);
   end Initialize_Request_Payload_Data_Private;

   procedure Initialize_Request_Payload_Data (Ctx : in out Context) is
   begin
      Initialize_Request_Payload_Data_Private (Ctx, RFLX_Types.To_Length (Field_Size (Ctx, F_Request_Payload_Data)));
   end Initialize_Request_Payload_Data;

   procedure Set_Answer_Payload_Data (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Answer_Payload_Data));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_Answer_Payload_Data_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_Answer_Payload_Data)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_Answer_Payload_Data)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_Answer_Payload_Data))) = Data);
   end Set_Answer_Payload_Data;

   procedure Set_Request_Payload_Data (Ctx : in out Context; Data : RFLX_Types.Bytes) is
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (Field_First (Ctx, F_Request_Payload_Data));
      Buffer_Last : constant RFLX_Types.Index := Buffer_First + Data'Length - 1;
   begin
      Initialize_Request_Payload_Data_Private (Ctx, Data'Length);
      pragma Assert (Buffer_Last = RFLX_Types.To_Index (Field_Last (Ctx, F_Request_Payload_Data)));
      Ctx.Buffer.all (Buffer_First .. Buffer_Last) := Data;
      pragma Assert (Ctx.Buffer.all (RFLX_Types.To_Index (Field_First (Ctx, F_Request_Payload_Data)) .. RFLX_Types.To_Index (Field_Last (Ctx, F_Request_Payload_Data))) = Data);
   end Set_Request_Payload_Data;

   procedure Generic_Set_Answer_Payload_Data (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Answer_Payload_Data);
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (First);
      Buffer_Last : constant RFLX_Types.Index := RFLX_Types.To_Index (First + RFLX_Types.To_Bit_Length (Length) - 1);
   begin
      Process_Answer_Payload_Data (Ctx.Buffer.all (Buffer_First .. Buffer_Last));
      Initialize_Answer_Payload_Data_Private (Ctx, Length);
   end Generic_Set_Answer_Payload_Data;

   procedure Generic_Set_Request_Payload_Data (Ctx : in out Context; Length : RFLX_Types.Length) is
      First : constant RFLX_Types.Bit_Index := Field_First (Ctx, F_Request_Payload_Data);
      Buffer_First : constant RFLX_Types.Index := RFLX_Types.To_Index (First);
      Buffer_Last : constant RFLX_Types.Index := RFLX_Types.To_Index (First + RFLX_Types.To_Bit_Length (Length) - 1);
   begin
      Process_Request_Payload_Data (Ctx.Buffer.all (Buffer_First .. Buffer_Last));
      Initialize_Request_Payload_Data_Private (Ctx, Length);
   end Generic_Set_Request_Payload_Data;
